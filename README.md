...
The client stores each version under `/var/lib/lan-distribution/client/datasets/<name>/versions/<SHA-256-manifest>/` and atomically switches `current` after the full TAR archive and every checksum have passed validation. An unchanged manifest is a NOOP. The stable target is a symlink to `current`; the client refuses to replace an unrelated existing file, directory, or symlink. `retain` keeps recent versions. Read and execute mode bits for regular files and directories are preserved; write bits are removed. Files are owned by root, with an optional local `group` for non-root application readers. Choose source modes such as `0640` for files and `0750` for directories when using a group. Applications can traverse the replica path when file modes permit it; credentials remain in a private directory. On connection failures, existing versions remain usable, and the daemon retries with a little jitter. Hooks run as an argv list after successful updates only, without a shell. Hook output and status go to the journal. A failed hook is reported while the newly installed version remains active.
