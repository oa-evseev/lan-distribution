...
| `GET /v1/datasets/<name>/archive` | mTLS and dataset grant | Returns an uncompressed TAR snapshot for a source-backed dataset. |
| `GET /v1/datasets/<name>/archive/<version>` | mTLS and dataset grant | Returns the immutable snapshot identified by a previously received manifest. |
...
The archive contains exactly one TAR member per manifest entry, with matching types, paths, sizes, and file checksums. Paths are relative POSIX paths. Absolute paths, `..`, `.`, repeated separators, backslashes, NUL bytes, symlinks, links, devices, and unknown member types are rejected. Source hard links are also rejected. The client never uses `extractall`. It stages the complete version, creates files with mode `0000` while writing, applies the exact metadata mode only after content verification, then atomically replaces the `current` symlink. The client requests `/archive/<version>` after reading a manifest. Published datasets resolve that request only in the immutable server-side version store, never through its mutable `current` link, so publication cannot mix payloads from two versions. Source-backed datasets retain their existing on-demand behavior; a source race is detected by the version/checksum validation and retried on a later cycle. The old local version stays active on failure.

## Published datasets

A source-backed dataset is configured as a path value in `[datasets]`. A published dataset is configured as `[datasets.<name>]` with `published=true` and has no configured source directory. `lan-distribution-server publish <dataset-name> <source-directory>` validates the source using the normal snapshot rules, creates a complete private copy under `<server-state>/datasets/<dataset-name>/versions/<version-id>/`, fsyncs it, and atomically replaces `<server-state>/datasets/<dataset-name>/current` with `versions/<version-id>`. The version ID is the normal canonical manifest hash, including ordinary permission metadata. Files are initially created with mode `0000`; uid/gid, ACLs, xattrs, capabilities, and special mode bits are not retained. An identical publication leaves `current` untouched. This release retains all complete versions, including the current and prior versions, to keep in-flight version-qualified payload requests available.
